...
class Ghost {
	...
	generateCoords() {
		...
		*/
		...
		function randomizeDirection(ghost) {
			...
			const spots = [MAP[ghost.row][ghost.col + 1], MAP[ghost.row][ghost.col - 1], MAP[ghost.row + 1][ghost.col], MAP[ghost.row - 1][ghost.col]];
			...
			if (moves.length === 0) {				// if there are no moves, stay in the current spot
		...
		if (this.row === 7 && this.col === 9 && MAP[this.row - 1][this.col] !== 2 && MAP[this.row - 1][this.col] <= 4) {
			this.direction = 'up'
		...
		switch (this.direction) {
			case 'right':
				if (MAP[this.row][this.col + 1] !== 2 && MAP[this.row][this.col + 1] <= 4) {
			...
			break;
			case 'up':
				if (MAP[this.row - 1][this.col] !== 2 && MAP[this.row - 1][this.col] <= 4) {
					return [this.row - 1, this.col]
				} else {
					return randomizeDirection(this)
				}
	...
	move() {
		...
		if (MAP[row][col] <= 4 && MAP[row][col] !== 2) { // you can move there
			if (this.coin) {
...
var pacman = {
	...
	move: function () {
		...
		if ((pacman.direction === 'right') && (MAP[pacman.row][pacman.col + 1] !== 2)) {
			...
			new_spot = true
		} else if ((pacman.direction === 'left') && (MAP[pacman.row][pacman.col - 1] !== 2)) {
			MAP[pacman.row][pacman.col] = 3;
			pacman.col --;
			new_spot = true
		} else if ((pacman.direction === 'up') && (MAP[pacman.row - 1][pacman.col] !== 2)) {
			MAP[pacman.row][pacman.col] = 3;
			pacman.row --;
			new_spot = true
		} else if ((pacman.direction === 'down') && (MAP[pacman.row + 1][pacman.col] !== 2)) {
			MAP[pacman.row][pacman.col] = 3;
			pacman.row ++
			new_spot = true
		}
		if (new_spot) {
			if (MAP[pacman.row][pacman.col] === 4) { // if the spot pacman is in is a coin, add points
				CURRENTSCORE += 10;
			}
...
function gameOver() {
	if (CURRENTSCORE === 1800) {return true}
	for (var i = 0; i < GHOSTS.length; i++) {
		if (pacman.row === GHOSTS[i].row && pacman.col === GHOSTS[i].col && !(GHOSTS[i].eatable)) {
			console.log('true')
			return true;
			};
	};
...
// 	GHOSTS.forEach(function (item, index, array) {	// item is a ghost object
// 		//console.log('pacman location', pacman.row, pacman.col, 'and ghost location', item.row, item.col)
// 		if (pacman.row === item.row && pacman.col === item.col) {
// 			console.log('true')
// 			return true;
// 		}
// 	});
...
// 	$('#game-over-text').css('display', 'none')
// 	$('#maze-container').css('opacity', '1');
// 	$('#game-over-screen').css('display', 'none');
// 	$('#game-over-text').addClass('blink');
...
// 	pacman.row = 13;
// 	pacman.col = 9;
// 	GHOSTS = [new Ghost('red', 5, 7, 8), new Ghost('pink', 6, 7, 9), new Ghost('blue', 7, 7, 10), new Ghost('orange', 8, 6, 9)];
...
function start () {
	...
	setTimeout(function(){$('#start-screen').hide(); game()}, 4000)
}
...
function game() {
	...
	setInterval(function () {
		...
		if (gameOver()) {
			...
			$('.more').css('display', 'none');
			document.getElementById('score').innerHTML = 'Score: ' + CURRENTSCORE.toString();
			setTimeout(function() {
				$('#game-over-screen').css('display', 'block');
